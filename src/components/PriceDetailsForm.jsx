...
export default function PriceDetailsForm({ initialDetails = {}, onClose, onSave }) {
  const [amount, setAmount] = useState(initialDetails.amount || '');
  const [category, setCategory] = useState(initialDetails.category || 'duration');
    ...
    <div className="modal-backdrop">
      <div className="price-details-form">
        ...
        <div className="form-group">
          <label>Unit</label>
          {category === 'custom' ? (
            <input type="text" value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="Enter custom unit" required/>
          ) : (
